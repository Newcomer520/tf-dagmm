...
def summary_report(inference_fn,
    ...
    ax.scatter(train_results.z_0, train_results.z_1, train_results.z_2, c=train_results.energy, cmap=plt.cm.get_cmap('jet'))
    ...
    ax.hist(test_OK_results.energy, color='blue', label='OK', bins=20)
    ax.hist(test_NG_results.energy, color='red', label='NG', bins=20)
    ax.legend()

    plt.figure(4)
    plt.step(recall, precision, color='b', alpha=0.2, where='post')
    plt.fill_between(recall, precision, step='post', alpha=0.2, color='b')
    plt.plot([0, 1], [precision[0], precision[0]], '--', linewidth=1, color='red')
    plt.text(0.3, precision[0] - 0.05, '{:.2f}%, fp: {:0d}'.format(precision[0] * 100, false_positive), color='red')
    plt.xlabel('Recall')
    plt.ylabel('Precision')
    plt.ylim([0.0, 1.05])
    plt.xlim([0.0, 1.0])
    # plt.title('PR curve: AP={:0.2f}, precision={:0.3f} as recall=1.0'.format(average_precision, precision[0]))
    ...
    if not saved_in:
        ...
        plt.savefig(os.path.join(saved_in, 'pr_curve.png'))
